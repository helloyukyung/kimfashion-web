  ...
  return (
    <div style={{left: `${x}%`, top: `${y}%`}} className="z-100 absolute">
      <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        <button type="button" className="w-9 flex h-9 items-center justify-center rounded-full bg-[var(--primary-01)]">
          <FaPlus className="text-white" />
        </button>
        {actualIsHovered && (
          <div className={`absolute ${tooltipLocation} p-3`}>
            <div className="bg-white p-3 flex h-[100px] w-[250px] gap-4 rounded">
